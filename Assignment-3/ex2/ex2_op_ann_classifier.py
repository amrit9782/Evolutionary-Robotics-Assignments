...
MUTATION_RATE = 0.15            # Chance for each gene to mutate
MUTATION_STRENGTH = 0.3         # Std. dev. of mutation
ELITISM_SIZE = 5                # Keep the 2 best individuals
...
# helper function
def activation_phi(x):
    """ The activation function given in the assignment """
    # Activation function: phi(x) = 2 / (1 + exp(-2x)) - 1
    return (2 / (1 + np.exp(-2 * x))) - 1

# -- ANN Output --
def ann_output_multilayer(genome, x, y):
    ''' Calculate output of a multilayer ANN '''

    h1_w0, h1_w1, h1_w2 = genome[0:3]
    h2_w0, h2_w1, h2_w2 = genome[3:6]
    o_w0,  o_w1,  o_w2  = genome[6:9]

    # Hidden Layer 1
    net_h1 = h1_w0 + h1_w1 * x + h1_w2 * y
    out_h1 = activation_phi(net_h1)

    # Hiddem Layer 2
    net_h2 = h2_w0 + h2_w1 * x + h2_w2 * y
    out_h2 = activation_phi(net_h2)

    # final output layer
    net_o = o_w0 + o_w1 * out_h1 + o_w2 * out_h2
    final_output = activation_phi(net_o)

    return final_output

# -- fitness function --
def calc_fitness(genome, data_x, data_y, data_class):
    """ Calculate the fitness of a single multilayer genome
     by comparing with the correct number of predicted classes """
    ann_outputs = ann_output_multilayer(genome, data_x, data_y)
...
def plot_classifier_multilayer(best_genome, data):
    ''' Plots the data points and the non-linear decision boundary
    by creating a background contour plot '''
    ...
    plt.scatter(class_0['x'], class_0['y'],
                label="Class 0", marker='o', alpha=0.7, edgecolors='k')
    
    plt.scatter(class_1['x'], class_1['y'],
                label="Class 1", marker='s', alpha=0.7, edgecolors='k')

    # Create a grid of points to sample the decision space
    ...
    # Get ANN output for every point on the grid
    grid_outputs = ann_output_multilayer(best_genome, xx.ravel(), yy.ravel())
    
    # Reshape the outputs back into a grid
    Z = grid_outputs.reshape(xx.shape)
    
    # Plot the decision boundary
    # This is the line between "Class 0" (<0) and "Class 1" (>0)
    plt.contour(
        xx, yy, Z, levels=[0.0],
        colors='r', linestyles='--'
    )
    # Fill the background to show the regions
    plt.contourf(
        xx, yy, Z, levels=[-1.0, 0.0, 1.0],
        colors=['#8888ff', '#ff8888'], alpha=0.3
    )
...
def main():
    ...
    # Show results
    print("\n--- Final Results (Multilayer) ---")
    print(f"Best Fitness Found: {best_fitness_overall} / {max_fitness} correct")
    print(f"Best Genome (9 weights):")
    print(np.array2string(best_genome_overall, precision=4))

    # --- 5. Plot Results ---
    plot_fitness_progress(log_best_fitness, log_avg_fitness)
    plot_classifier_multilayer(best_genome_overall, data)
